...
// ✅ Función para filtrar por número total de huéspedes
function filtrarPorHuespedes() {
  const totalHuespedes = adults + children + babies;

  if (totalHuespedes === 0) {
    renderCards(stays, contenedor); // Mostrar todos si no hay selección
    return;
  }
...
function actualizar() {
  ...
  filtrarPorHuespedes(); // 👉 Llamamos la función al actualizar
}
