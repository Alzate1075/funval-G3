...
export function agregarFiltroInput(input, stays, contenedor, modal1) {
  const listaFiltrada = document.getElementById("cityList");
  ...
  input.addEventListener("input", () => {
    ...
    // Filtra las ciudades que contienen el texto escrito
    const ciudadesFiltradas = ciudadesUnicas.filter(city =>
      city.toLowerCase().includes(texto)
    );

    ciudadesFiltradas.forEach(city => {
      const btn = document.createElement("button");
      btn.innerHTML = `
        <div class="flex items-center gap-2 p-2 hover:bg-gray-100 rounded-lg transition">
          <img src="./src/Jpg/icons/ubicacion.png" alt="ubicacionLogo" class="h-4 w-4" />
          <span class="text-gray-600 font-medium">${city}, Finland</span>
        </div>
      `;
